...
public class SpecialityChefLinkController {
	...
	@PostMapping("/addLink")
	public ResponseEntity<Speciality_ChefLink> addLink(@RequestBody Map<String, Map<String, String>> request){
		try {
			UUID chefId = UUID.fromString(request.get("chef").get("chefId"));
			UUID specialityId = UUID.fromString(request.get("chefSpeciality").get("specialityId"));
